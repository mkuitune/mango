...
static bool
s_recv_str (void *socket, std::vector<char>& str) {
	int buflen = str.size();
    int size = zmq_recv (socket, &str[0], buflen, ZMQ_DONTWAIT);
   // int size = zmq_recv (socket, &str[0], buflen, 0);
    if (size == -1)
        return false;
    if (size >= buflen)
        size = (buflen - 1);
...
static int
s_send_noblock(void *socket, const char *string) {
